...
export default function SuperAdminSupport() {
  ...
  const [savingInquiry, setSavingInquiry] = useState<string | null>(null);
  ...
  async function loadTickets() {
    try {
      ...
      setTicketsError(e instanceof Error ? e.message : "Failed to load");
    } finally {
  ...
  async function loadInquiries() {
    try {
      ...
      setInquiriesError(e instanceof Error ? e.message : "Failed to load");
    } finally {
      setInquiriesLoading(false);
    }
  }

  useEffect(() => { loadTickets(); loadInquiries(); }, []);
  ...
  async function handleInquiryStatus(id: string, status: ContactInquiry["status"]) {
    setSavingInquiry(id);
    try {
      await updateInquiryStatus(id, status);
      setInquiries((prev) => prev.map((i) => i.id === id ? { ...i, status } : i));
      toast.success(status === "resolved" ? "Marked resolved" : "Marked as read");
    } catch {
    ...
    <div>
      ...
      <div className="flex gap-2 mb-5">
        <button
          ...
          className={`px-5 py-2 rounded-xl text-[13px] font-bold cursor-pointer transition-all border ${
            tab === "tickets"
        ...
        <button
          ...
          className={`px-5 py-2 rounded-xl text-[13px] font-bold cursor-pointer transition-all border relative ${
            tab === "inquiries"
              ? "bg-amber-500 text-white border-amber-500"
              : "bg-white text-ink-muted border-border-default hover:bg-warm-50"
          }`}
        ...
        <>
          {ticketsLoading ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-[14px] p-6 text-center">
              <div className="text-red-600 font-bold mb-2">⚠️ {ticketsError}</div>
              <button onClick={loadTickets} className="mt-2 px-4 py-2 bg-amber-500 text-white rounded-xl text-[11px] font-bold cursor-pointer">Retry</button>
            </div>
          ) : (
            <>
      ...
      {tab === "inquiries" && (
        <>
          {inquiriesLoading ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-[14px] p-6 text-center">
              <div className="text-red-600 font-bold mb-2">⚠️ {inquiriesError}</div>
              <button onClick={loadInquiries} className="mt-2 px-4 py-2 bg-amber-500 text-white rounded-xl text-[11px] font-bold cursor-pointer">Retry</button>
            </div>
          ) : (
            <>
              <div className="flex gap-2.5 flex-wrap mb-4">
                <StatCard icon="🆕" label="New" value={String(inquiries.filter(i => i.status === "new").length)} accent="text-red-500" />
                <StatCard icon="👁️" label="Read" value={String(inquiries.filter(i => i.status === "read").length)} accent="text-amber-600" />
                <StatCard icon="✅" label="Resolved" value={String(inquiries.filter(i => i.status === "resolved").length)} accent="text-green-600" />
                <StatCard icon="📊" label="Total" value={String(inquiries.length)} />
              </div>

              {inquiries.length === 0 ? (
                <div className="text-center py-12 text-ink-muted text-sm">
                  No contact inquiries yet. They will appear here when someone fills the contact form.
                </div>
              ) : (
                <div className="space-y-2.5">
                  {inquiries.map((inq) => (
                    <div key={inq.id} className={`bg-white rounded-[14px] border border-l-4 border-border-default ${inq.status === "new" ? "border-l-red-500" : inq.status === "read" ? "border-l-yellow-400" : "border-l-green-400"}`}>
                      <div
                        className="p-4 cursor-pointer"
                        onClick={() => {
                          setExpandedInquiry(expandedInquiry === inq.id ? null : inq.id);
                          if (inq.status === "new") handleInquiryStatus(inq.id, "read");
                        }}
                      >
                        <div className="flex flex-wrap justify-between gap-2">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-1.5 mb-1">
                              <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${INQUIRY_STATUS_STYLE[inq.status] ?? ""}`}>
                                {inq.status}
                              </span>
                              <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-blue-100 text-blue-700">
                                {inq.user_type}
                            ...
                            <div className="text-[13px] font-bold text-ink">{inq.name}</div>
                            <div className="text-[11px] text-ink-muted mt-0.5">
                              {inq.contact} · {new Date(inq.created_at).toLocaleDateString("en-IN")}
                              <span className="ml-1 italic"> · {inq.message.slice(0, 60)}{inq.message.length > 60 ? "…" : ""}</span>
                            </div>
                          </div>
                          <span className="text-ink-muted text-[13px] self-center">{expandedInquiry === inq.id ? "▲" : "▼"}</span>
                        </div>
                      </div>
                      ...
                      {expandedInquiry === inq.id && (
                        <div className="px-4 pb-4 border-t border-border-light">
                          <div className="pt-3 space-y-3">
                            <div className="bg-warm-50 rounded-xl p-3">
                              <div className="text-[10px] font-bold text-ink-muted mb-1">MESSAGE</div>
                              <div className="text-[12px] text-ink">{inq.message}</div>
                            </div>
                            <div className="flex items-center gap-3 text-[12px] text-ink-muted flex-wrap">
                              <span><span className="font-bold text-ink">Name:</span> {inq.name}</span>
                              <span><span className="font-bold text-ink">Contact:</span> {inq.contact}</span>
                              <span><span className="font-bold text-ink">Type:</span> {inq.user_type}</span>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {inq.status !== "resolved" && (
                                <button
                                  onClick={() => handleInquiryStatus(inq.id, "resolved")}
                                  disabled={savingInquiry === inq.id}
                                  className="px-3 py-1.5 rounded-xl bg-green-500 text-white text-[11px] font-bold cursor-pointer hover:bg-green-600 transition-colors disabled:opacity-50"
                                >
                                  {savingInquiry === inq.id ? "Saving..." : "✅ Mark Resolved"}
                                </button>
                              )}
                              {inq.contact.includes("@") ? (
                                <a
                                  href={`mailto:${inq.contact}`}
                                  className="px-3 py-1.5 rounded-xl bg-amber-500 text-white text-[11px] font-bold cursor-pointer hover:bg-amber-600 transition-colors"
                                >
                                  📧 Reply via Email
                                </a>
                                ...
                                <a
                                  href={`https://wa.me/91${inq.contact.replace(/\D/g, "")}`}
                                  target="_blank"
