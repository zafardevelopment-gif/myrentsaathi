      ...
      <div className="bg-white rounded-[20px] w-full max-w-md max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <tr key={i} className="border-b border-border-light">
                  ...
                  <td className="py-1">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                      r.status === "created" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                    }`}>
                      {r.status === "created" ? "OK" : "Skipped"}
                    </span>
                  </td>
                </tr>
              ))}
        ...
        {skipped > 0 && (
          <div className="mx-5 mt-3 bg-orange-50 border border-orange-200 rounded-xl p-3 text-[11px] text-orange-700">
            <b>{skipped} records not inserted due to plan limit.</b>
...
export default function AdminFlats() {
  ...
  async function handleAddFlat(e: React.FormEvent) {
    ...
    try {
      // 1. Check landlord limit BEFORE creating anything
      const wantsLandlord = !!(form.owner_name && form.owner_phone);
      if (wantsLandlord && landlordStats) {
        if (landlordStats.count >= landlordStats.limit) {
          const remaining = 0;
          toast.error(
            `Your plan allows ${landlordStats.limit} landlords. You've reached the limit.\nContact admin to purchase more slots.`,
            { duration: 5000 }
          );
          setSaving(false);
          return;
      ...
      // 3. Create landlord user (if owner details provided)
      ...
      // 4. Create tenant user (if tenant details + landlord known)
      ...
      if (wantsTenant && landlordUserId) {
        ...
        // Just store text — no tenant account without landlord
  ...
  function downloadSampleCSV() {
    ...
    const sampleRow = ["101", "A", "1", "2BHK", "900", "15000",
      "Rajesh Kumar", "+91-98765-11111", "rajesh@example.com", "Amit Singh", "+91-98765-22222", "amit@example.com"];
  ...
  async function handleBulkUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (data.length === 0) { toast.error("Invalid CSV format"); return; }
    setPreviewData(data);
    setShowPreview(true);
  ...
  async function confirmBulkImport() {
    ...
    try {
      // Check how many landlord slots are available
      ...
      for (const row of previewData) {
        const flatLabel = `${row.flat_number}${row.block ? ` (${row.block})` : ""}`;
        ...
        // Create landlord if within limit
        let landlordUserId: string | null = null;
        if (wantsLandlord) {
          if (availableLandlordSlots > 0) {
            const lResult = await addLandlordBySocietyAdmin({
            ...
            result.status = "landlord_skipped";
            totalSkipped++;
          }
        }
      ...
      setFlats(f);
      const newStats = await getSocietyLandlordStats(societyId);
  ...
  function downloadResultsCSV() {
    ...
    const headers = ["flat_number", "block", "owner_name", "owner_user_id", "owner_password",
      "tenant_name", "tenant_user_id", "tenant_password", "status"];
    const rows = bulkResults.results.map(r =>
      [r.flat_number, r.block, r.owner_name, r.owner_user_id, r.owner_password,
       r.tenant_name, r.tenant_user_id, r.tenant_password, r.status].join(",")
    );
    ...
    <div>
      ...
      {showPreview && (
        <div className="bg-white rounded-[14px] p-4 border border-yellow-300 mb-4">
          <div className="text-sm font-bold text-ink mb-1">📋 Import Preview ({previewData.length} flats)</div>
          {landlordStats && (
            <div className="text-xs text-ink-muted mb-3">
              Available landlord slots: <b className={slotsLeft === 0 ? "text-red-500" : "text-green-600"}>{slotsLeft}</b>
              {slotsLeft !== null && previewData.filter(r => r.owner_name).length > slotsLeft && (
                <span className="ml-1 text-red-500">
                  — {previewData.filter(r => r.owner_name).length - slotsLeft!} landlord(s) will be skipped
                </span>
              )}
            </div>
          )}
                      ...
                      <>
                        <div>
                          ...
                          <div className="space-y-2">
                            ...
                            <input className={inputClass} placeholder="Phone" value={(editFormData.owner_phone ?? flat.owner_phone) || ""} onChange={e => setEditFormData(prev => ({ ...prev, owner_phone: e.target.value }))} disabled={!!linkedOwner} />
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            ...
                            <input className={inputClass} placeholder="Phone" value={(editFormData.tenant_phone ?? flat.tenant_phone) || ""} onChange={e => setEditFormData(prev => ({ ...prev, tenant_phone: e.target.value }))} disabled={!!linkedTenant} />
