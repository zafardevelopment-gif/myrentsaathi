...
import {
  getLandlordTickets, getLandlordSocietyId, createLandlordTicket,
  type LandlordTicket,
...
export default function LandlordComplaints() {
  ...
  const [form, setForm] = useState({
    category: "plumbing", subject: "", description: "", priority: "medium",
  });
  ...
  useEffect(() => {
    ...
    async function init() {
      const [sid] = await Promise.all([
        getLandlordSocietyId(user!.email).catch(() => null),
        loadTickets(),
      ]);
      setSocietyId(sid);
      setLoading(false);
  ...
  async function handleCreate() {
    ...
    if (!societyId) { toast.error("Unable to identify your society. Contact admin."); return; }
    setSaving(true);
    try {
      await createLandlordTicket(user!.email, societyId, form);
      toast.success("Complaint submitted to society!");
      setShowForm(false);
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-[14px] p-4 border border-border-default mb-4 space-y-3">
          <div className="text-[13px] font-bold text-ink">New Complaint to Society</div>
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-2.5 text-[11px] text-blue-700">
            This complaint will be sent to the society management.
          </div>
          <input
            className={inputClass}
            placeholder="Subject (e.g. Security issue near parking)"
            value={form.subject}
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {[
          ...
          <button
            key={s.key}
            onClick={() => { setFilterStatus(s.key as typeof filterStatus); setPage(1); }}
      ...
      <div className="bg-white rounded-[14px] border border-border-default p-3.5 mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {hasFilters && (
            <button onClick={resetFilters} className="px-3 py-2 rounded-xl border border-red-200 text-red-500 text-xs font-semibold cursor-pointer">
              Clear
            </button>
          )}
        </div>
      ...
      {paged.length === 0 ? (
        <div className="text-center py-12 text-ink-muted text-sm">
          ...
          {hasFilters ? "No complaints match your filters." : "No complaints raised yet."}
        </div>
      ) : (
          ...
          {paged.map(tk => (
            <div key={tk.id} className={`bg-white rounded-[14px] p-4 border border-border-default border-l-4 ${tk.source === "tenant" ? "border-l-blue-400" : "border-l-orange-400"}`}>
              ...
              <div className="flex gap-2 flex-wrap">
                {tk.status === "open" && (
                  <button
                    onClick={() => updateStatus(tk.id, "in_progress")}
                ...
                {(tk.status === "open" || tk.status === "in_progress") && (
                  <button
                    onClick={() => updateStatus(tk.id, "resolved")}
                ...
                {tk.status === "resolved" && (
                  <span className="text-[11px] text-green-600 font-semibold">✓ Resolved</span>
                )}
              </div>
            </div>
          ))}
