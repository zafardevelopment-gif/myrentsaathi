          ...
          return (
            <div key={rp.id} className="bg-white rounded-[14px] p-4 border border-border-default mb-2 flex justify-between items-center gap-3">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center text-sm font-extrabold text-brand-500">{initials}</div>
                <div>
                  <div className="text-sm font-bold text-ink">{tenantName}</div>
                  <div className="text-[11px] text-ink-muted">
                    {flat ? `Flat ${flat.flat_number}${flat.block ? ` (${flat.block})` : ""}` : "—"}
                    {showAll && rp.month_year ? ` · ${new Date(rp.month_year + "-01").toLocaleString("en-IN", { month: "short", year: "numeric" })}` : ""}
