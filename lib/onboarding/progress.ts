...
async function landlordSteps(landlordId: string): Promise<SetupStep[]> {
  ...
  return [
    ...
    { key: "bank", title: "Add Bank Account", required: true, href: "/landlord/settings?section=bank",
...
async function societySteps(societyId: string): Promise<SetupStep[]> {
  ...
  return [
    ...
    { key: "bank", title: "Add Society Bank Account", required: true, href: "/admin/settings?section=bank",
      status: bankCount > 0 ? "completed" : "not_started" },
